...
//  초기값 셋팅   
var fvs_default_detail = "OUT_GU^O02@OVERSEA_GU^1@ORDER_PSPEC^.@SUJU_STS^1@MISAYU^N@AMTGU^Y@TUNCU^KRW@RCV_GUBUN^1@CRT_PGMID^1@GWSTS^00@GWGBN^N@DANGBN^3@MEDIYN^N@MEDMODIYN^N";
...
var fvs_default_master = "MISAYU^N@DEPOT_NO^ZA010@SUGUGB^1@GWGBN^N";
...
this.ff_SetCondition = function () {
    ...
    // 20231012_의약품 단가 수정 권한 확인을 위한 접속 아이디 추가
    this.ds_head.setColumn(0, "ARG_LOGIN_EMPNO", ivUserid);
...
this.btn_add_onclick = function (obj: Button, e: nexacro.ClickEventInfo) {
    ...
    this.Div_content.DIV_package.ed_conspcgbn.set_value('');

    //위치 수정 this.ff_Screen(); 뒤로 가면 마스터체인지 이벤트 발생으로 창고 에러
    var vJi_empno = this.Div_head.ed_ji_empno.value;
    ...
    this.ds_master.setColumn(0, "ORD_CANCEL_CAUSE", 'N');
    this.ds_master.set_enableevent(true);

    this.ds_trans.setColumn(0, "TRANS_CGBN", 'H01');//택배사 기본값(한진)
    ...
    this.Div_detail.btn_detail_mob_ok.set_visible(false);      // 모바일 접수
    this.Div_detail.btn_detail_mob_cncl.set_visible(false);    // 모바일 취소
    this.Div_detail.btn_detail_add00.set_enable(true);
    this.Div_detail.btn_detail_delete00.set_enable(true);
...
this.btn_save_onclick = function (obj: Button, e: nexacro.ClickEventInfo) {
    ...
    if (nChk == 0) {
        ...
        if (!NXCore.isEmpty(vEstno)) {
            ...
            var vOrder_date = this.ds_master.getColumn(0, "ORDER_DATE");

            var vRtn = this.gf_SelectSql_sync("ds_temp: SELECT SUBSTR(A.SAREA, 1, 2),  NVL(B.VALIDAMT, 0) FROM VNDMST_SUB A, VNDDAMBO B "
                + " WHERE A.CVCOD = '" + vCvcod + "' AND A.CVCOD = B.CVCOD(+) AND B.GUBUN(+) = '1' AND B.ALLOWDT(+) >= '" + vOrder_date + "' ", "SELECT_VNDMST_SUB", "ff_Callback_sync", 0);
            if (vi_ErrorCode < 0) return false;
            ...
            var nTot_chul = this.ff_Danpum_Limit2(vCvcod);

            if (nTot_chul == -1) return;
        ...
        var vNapsame = this.ds_master.getColumn(0, "ORD_CANCEL_CAUSE");
        ...
        for (i = 0; i < this.ds_detail.rowcount; i++) {
            ...
            if (vSts == Dataset.ROWTYPE_INSERT) {
                ...
                this.ds_detail.setColumn(i, "ORD_CANCEL_CAUSE", vNapsame);
...
this.ff_Callback = function (sSvcID, ErrorCode, ErrorMsg) {
    ...
    switch (sSvcID) {
        case "SELECT_LIST":
            ...
            var vs_trans_name = this.ds_head.getColumn(0, 'ARG_TRANS_NAME');
            if (NXCore.isEmpty(this.ds_head.getColumn(0, 'ARG_TRANS_NAME')) || vs_trans_name == '') {
                break;
            }
            else {
                this.ds_list.set_filterstr("TRANS_NAME == '" + vs_trans_name + "'");
            }
...
this.ff_Tran = function (strSvcId) {
    ...
    switch (strSvcId) {
        ...
        case "SAVE_LIST":   //
            ...
            v_SvcAct = "si/send/si_send_order_neo_e_q_1tr.jsp?dbconn=0";
            v_InDataset = "input1=ds_list:U";       // 반드시 input1으로 기술할것
            v_OutDataset = "";
            break;
...
this.ff_Tran_sync = function (strSvcId) {
    ...
    switch (strSvcId) {
        ...
        case "SELECT_PKGQTY":   // 품목분류별 수량 조회 
            ...
            break;
        // 		case "SAVE_MASTER" :
        // 				v_SvcAct		= "si/pkg/si_pkg_master_e_23tr.jsp?dbconn=" + 0;
...
this.ds_list_oncolumnchanged = function (obj: Dataset, e: nexacro.DSColChangeEventInfo) {
    ...
    switch (vName) {
        case "ORD_CANCEL_CHK":
            if (vData == '1') {
                this.ds_list.setColumn(vRow, "ORD_CANCEL_CAUSE", 'Y');
            }
            else {
                this.ds_list.setColumn(vRow, "ORD_CANCEL_CAUSE", 'N');
            }
            break;
    }
...
this.ds_detail_oncolumnchanged = function (obj: Dataset, e: nexacro.DSColChangeEventInfo) {
    ...
    switch (vName) {
        ...
        case "ITDSC":
            ...
            var vSql_2 = "SELECT DEPTCODE FROM  ERPACC.P1_MASTER WHERE EMPNO = '" + application.gvs_empid + "'";

            var vRtn_2 = this.gf_SelectSql_sync("ds_temp:" + vSql_2, "SELECT_DEPTCODE", "ff_Callback_sync", 0);

            var vs_deptcode = vRtn_2[1];
            ...
            if (NXCore.isEmpty(this.ds_master.getColumn(0, 'ESTNO')) && this.ds_master.getColumn(0, 'SAREA') != '01' && vs_deptcode != 'P125000')//단품 출고제한 로직 (단품 주문시 상품, KIT 제품은 주문 불가)
            {
                var v_danga = 0;
                if (vRtn[5] == '7') {
                    alert("단품 주문시 상품은 주문 불가합니다.");
                    vDw.setColumn(vRow, "ITNBR", null);
                    vDw.setColumn(vRow, "PRODNM", null);
                    vDw.setColumn(vRow, "ITDSC", null);
                    vDw.setColumn(vRow, "ISPEC", null);
                    ...
                    v_danga = this.ff_Danga_re(vReturnSale[1], vRow, 'KRW', 'Auto');

                    if (v_danga >= 500000) //kit 제한 (기준단가 50만원 이상 대상)
                    {
                        alert("단품 주문시 KIT 제품은 주문 불가합니다.");
                        vDw.setColumn(vRow, "ITNBR", null);
                        vDw.setColumn(vRow, "PRODNM", null);
                        vDw.setColumn(vRow, "ITDSC", null);
                        vDw.setColumn(vRow, "ISPEC", null);
            ...
            if (vDept == 'ZA010' || vDept == 'ZA094') {
                ...
                if (vRtn[5] == '1' && vRtn[6].substr(0, 4) == '0703') {
                    if (this.ds_package.getColumn(0, "ESTNO") != '' || !NXCore.isEmpty(this.ds_package.getColumn(0, "ESTNO"))) {
                        if (vCon_spc_gbn != '1' && vCon_spc_gbn != 'B' && vCon_spc_gbn != '5' && vCon_spc_gbn != '11' && vCon_spc_gbn != '13' && vCon_spc_gbn != '3' && vCon_spc_gbn != '6' && vCon_spc_gbn != '9') {
            ...
            var vSql = "SELECT	COUNT(*) AS CNT FROM REFFPF WHERE RFCOD = '2L' AND RFGUB <> '00' AND RFNA1 = '" + vReturnSale[1] + "' ";
            var vRtn = this.gf_SelectSql_sync("ds_temp:" + vSql, "SELECT_reffpf_2L", "ff_Callback_sync", 0);
            if (vi_ErrorCode < 0) return false;

            if (vRtn[1] > 0) {
                vDw.setColumn(vRow, "MEDIYN", "Y");
            } else {
                vDw.setColumn(vRow, "MEDIYN", "N");
            }

            var vSql = "SELECT	COUNT(*) AS CNT FROM REFFPF WHERE RFCOD = '2P' AND RFGUB <> '00' AND RFNA1 = '" + ivUserid + "' ";
            var vRtn = this.gf_SelectSql_sync("ds_temp:" + vSql, "SELECT_reffpf_2P", "ff_Callback_sync", 0);
            if (vi_ErrorCode < 0) return false;

            if (vRtn[1] > 0) {
                vDw.setColumn(vRow, "MEDMODIYN", "Y");
            } else {
                vDw.setColumn(vRow, "MEDMODIYN", "N");
            }
        ...
        case "ORDER_QTY":
            ...
            //단가 체크 품목인지 확인.
            var vs_estno = this.ds_master.getColumn(0, "ESTNO");	// 계약번호 ex). 202305260005
            //trace("vs_estno = "+vs_estno);
            if (!NXCore.isEmpty(vs_estno)) {
                var vs_dangachk = this.gf_SelectSql_sync("ds_temp: Select ITTYP, ITCLS from itemas where itnbr = '" + this.ds_detail.getColumn(vRow, 'ITNBR') + "'", "SELECT_DANGA_CHK", "ff_Callback_sync", 0);

                //this.Div_content.ed_cvcod.value	// 계약처

                if (vs_dangachk[1] == '1' && vs_dangachk[2].substr(0, 2) != '03' && vs_dangachk[2].substr(0, 4) != '0703')//체크대상
                // 1-완제품이고		//품목분류가 03(Tool)이 아니고		//품목분류가 0703(NeoGuide)가 아니면 
                {
                    var vDanga = this.ff_Danga_re(this.ds_detail.getColumn(vRow, 'ITNBR'), vRow, 'KRW', 'Auto');
                    var vAmt_chk1 = Math.floor(parseFloat(vData) * vDanga);//기준단가 금액
                    var vAmt_chk2 = Math.floor(parseFloat(vData) * vDw.getColumn(vRow, "ORDER_PRC"));//수정단가 금액
                    var vAmt_cal = vAmt_chk1 - vAmt_chk2;

                    //기준단가 대비 수정단가 계산금액이 20000원을 초과하여도 주문할 수 있도록 제한풀어주는 YK품목인지 체크. 
                    var vs_YKchk = this.gf_SelectSql_sync("ds_temp: select count(*) as cnt from REFFPF where rfcod='G1' and rfna1= '" + this.ds_detail.getColumn(vRow, 'ITNBR') + "'", "SELECT_YK_CHK", "ff_Callback_sync", 0);
                    if (vs_YKchk == 0)	// 0이면 제한풀어주는 YK제품이 아님. 1이면 제한풀어주는 YK제품이므로 주문접수할 수 있도록 풀어줌. 
                    {
                        if (vAmt_cal < -20000 || vAmt_cal > 20000) {
                            alert("기준단가 대비 수정단가 계산금액이 20000원을 초과하였습니다. 변경시 예상금액 = " + vAmt_chk2 + " 계산금액 차이 = " + vAmt_cal);
                            this.ds_detail.setColumn(vRow, "ORDER_QTY", vOld_data);
                            return;
                        }
                    }
        ...
        case "ORDER_PRC":
            ...
            //단가 체크 품목인지 확인.
            var vs_estno = this.ds_master.getColumn(0, "ESTNO");
            if (!NXCore.isEmpty(vs_estno)) {
                var vs_dangachk = this.gf_SelectSql_sync("ds_temp: Select ITTYP, ITCLS from itemas where itnbr = '" + this.ds_detail.getColumn(vRow, 'ITNBR') + "'", "SELECT_DANGA_CHK", "ff_Callback_sync", 0);
                ...
                {
                    var vDanga = this.ff_Danga_re(this.ds_detail.getColumn(vRow, 'ITNBR'), vRow, 'KRW', 'Auto');
                    var vAmt_chk1 = Math.floor(vDw.getColumn(vRow, "ORDER_QTY") * vDanga);//기준단가 금액
                    var vAmt_chk2 = Math.floor(vDw.getColumn(vRow, "ORDER_QTY") * parseFloat(vData));//수정단가 금액
                    var vAmt_cal = vAmt_chk1 - vAmt_chk2;
                    ...
                    if (vs_YKchk == 0)	// 0이면 제한풀어주는 YK제품이 아님. 1이면 제한풀어주는 YK제품이므로 주문접수할 수 있도록 풀어줌. 
                    {
                        if (vAmt_cal < -20000 || vAmt_cal > 20000) {
                            alert("기준단가 대비 수정단가 계산금액이 20000원을 초과하였습니다. 변경시 예상금액 = " + vAmt_chk2 + " 계산금액 차이 = " + vAmt_cal);
                            this.ds_detail.setColumn(vRow, "ORDER_PRC", vOld_data);
                            return;
                        }
                    }
                }
...
this.grd_list_oncelldblclick = function (obj: Grid, e: nexacro.GridClickEventInfo) {
    ...
    if (NXCore.isEmpty(vRtn[1]) || vRtn[1] == '' || vRtn[1] == 0) {// 주문 수정 가능 건.
        this.ff_protect_content(true);
        this.Div_content.set_enable(true);
        this.gd_detail.set_cssclass('');
        this.gd_detail.set_readonly(false);
        ...
        if (vRtn2[1] == 0 || NXCore.isEmpty(vRtn2[1]) || vRtn2[1] == '') {// 주문 수정 가능 건.
            this.Div_content.set_enable(true);
            this.gd_detail.set_cssclass('');
            this.gd_detail.set_readonly(false);
        }
    ...
    var vRtn3 = this.gf_SelectSql_sync("ds_temp: SELECT COUNT(*)  FROM SORDER_MAPPING WHERE ORDER_NO LIKE '" + vOrder_no + "%' ", "SELECT_reffpf_5A", "ff_Callback_sync", 0);

    if (vRtn3[1] != 0) {
        alert("의약품 출하대기 상태이므로 기본정보는 수정 할 수 없고 삭제도 안됩니다.");
        this.gf_mdi_btn_disable("delete,save");
    ...
    if (this.ds_temp.getColumn(0, "DANGA_LIMITYN") == 'Y') {
        ...
        this.gd_detail.setCellProperty("body", this.gd_detail.getBindCellIndex("body", "ORDER_PRC"), 'edittype', "expr:SUJU_STS == '4' ||  INVOICE_QTY > 0 || PICK_QTY > 0 || (MEDIYN == 'Y' && MEDMODIYN == 'N' ) ? 'none' :'masknumber'");
...
this.ff_Screen = function () {
    //	ivChk = '0';  // 화면모드 0 : List(조회), 1 : 신규입력, 2 : 수정 또는 삭제,
    if (ivChk == '0') {
        this.gf_mdi_btn_disable("delete,cancel");
        this.gf_mdi_btn_enable("query,save,excel_chg");
...
this.ff_UpdateChk = function () {
    ...
    if (NXCore.isEmpty(this.ds_master.getColumn(0, "ESTNO")) && this.ds_master.getColumn(0, 'SAREA') != '01' && vs_deptcode != 'P125000') {
        for (var i = 0; i < this.ds_detail.rowcount; i++) {
            var vSql_2 = "SELECT ITTYP FROM ITEMAS WHERE ITNBR = '" + this.ds_detail.getColumn(i, "ITNBR") + "'";
            var vRtn_2 = this.gf_SelectSql_sync("ds_temp:" + vSql_2, "SELECT_ITEMAS", "ff_Callback_sync", 0);
            if (vi_ErrorCode < 0) return false;
            ...
            if (vRtn_2[1] == '7') {
                ...
                v_danga = this.ff_Danga_re(this.ds_detail.getColumn(i, "ITNBR"), i, 'KRW', 'Auto');
    ...
    for (i = 0; i < this.ds_detail.rowcount; i++) {
        ...
        if (vSarea.substr(0, 2) == '01') {
            ...
            if (vIns_claim_yn == 'I') {
                if (vCon_spc_gbn != '1' && vCon_spc_gbn != '6' && vCon_spc_gbn != '7' && vCon_spc_gbn != '8') {
                    alert((i + 1) + "행 보험제품은 제품PAC, 소액, C_plus 로만 출고 가능합니다.");
                    return -1;
                }

                if (vTax_gbn1 == 'Y') {
                    alert((i + 1) + "행 계산서 수금분 발행 계약으로는 보험제품을 출고할 수 없습니다.");
                    return -1;
                }
            }
        }
        ...
        vSql = "SELECT  COUNT(RFNA1)	AS	CNT   FROM    REFFPF  WHERE   RFCOD   =   '2L'    AND RFGUB   <>  '00' AND RFNA1   =   '" + vItnbr + "'";
        vRtn = this.gf_SelectSql_sync("ds_temp:" + vSql, "SELECT_reffpf_2L", "ff_Callback_sync", 0);
        if (vi_ErrorCode < 0) return false;

        if (this.ds_temp.getColumn(0, "CNT") != '0' && vDepot_no != 'ZA081') {
            alert("의약품 형번은 출고창고를 의약품 창고로 주문 부탁드립니다");
            return -1;
        }

        vSql = "SELECT  RFNA6	AS	CNT   FROM    REFFPF  WHERE   RFCOD   =   '2L'    AND RFGUB   <>  '00' AND RFNA1   =   '" + vItnbr + "'";
        vRtn = this.gf_SelectSql_sync("ds_temp:" + vSql, "SELECT_reffpf_2L", "ff_Callback_sync", 0);
        if (vi_ErrorCode < 0) return false;

        if (!NXCore.isEmpty(vRtn[1]) && vRtn[1] != '') {
            var nQty = this.ds_detail.getColumn(i, "ORDER_QTY");
            var vEaCnt = nexacro.toNumber(nQty) % nexacro.toNumber(vRtn[1]);

            if (vEaCnt != 0) {
                alert("의약품 : " + vItnbr + "의 주문수량은 " + vRtn[1] + "개 단위로 입력하세요");
                return -1;
            }
        }
    ...
    var vSql2 = "SELECT PKGNO , AMOUNT, USE_COMM_SOMO_CON_AMOUNT, PKGGU, CON_SPC_GBN, IMPLANT_CNT,  "
        + " USE_COMM_SOMO_PRICE_AMOUNT, USE_SOMO_CON_AMOUNT_BACK, USE_SOMO_PRICE_AMOUNT_BACK, USE_COMM_MACH_PRICE_AMOUNT, USE_COMM_MACH_CON_AMOUNT, USE_MACH_PRICE_AMOUNT_BACK, USE_MACH_CON_AMOUNT_BACK , FIRST_TRANS FROM ESTIMATE_HEAD WHERE ESTNO = '" + vEstno + "'";
    ...
    if (this.ds_temp.getColumn(0, "PKGGU") == '03' || this.ds_temp.getColumn(0, "PKGGU") == '04') {
        if (NXCore.isEmpty(this.ds_temp.getColumn(0, "FIRST_TRANS")) || this.ds_temp.getColumn(0, "FIRST_TRANS") == 'N') {
            var nAmount_chul = this.ds_temp.getColumn(0, "AMOUNT");
            var nComm_rate = parseFloat(this.ds_temp.getColumn(0, "IMPLANT_CNT"));
            var nComm_amount = 0;
            nAmount_chul = nexacro.round(nAmount_chul * (nComm_rate / 100), 0); //출고 제한 금액
            nComm_amount = (this.ds_temp.getColumn(0, "USE_COMM_SOMO_CON_AMOUNT") + this.ds_temp.getColumn(0, "USE_COMM_SOMO_PRICE_AMOUNT") + this.ds_temp.getColumn(0, "USE_COMM_MACH_PRICE_AMOUNT") + this.ds_temp.getColumn(0, "USE_COMM_MACH_CON_AMOUNT"))
                - (this.ds_temp.getColumn(0, "USE_SOMO_PRICE_AMOUNT_BACK") + this.ds_temp.getColumn(0, "USE_SOMO_CON_AMOUNT_BACK") + this.ds_temp.getColumn(0, "USE_MACH_PRICE_AMOUNT_BACK") + this.ds_temp.getColumn(0, "USE_MACH_CON_AMOUNT_BACK")); //기 출고 상품금액
            var nComm_amount_org = nAmount_chul - nComm_amount;
            var nIttypCount = 0;

            for (var i = 0; i < this.ds_detail.rowcount; i++) {
                var vs_Itnbr = this.ds_detail.getColumn(i, "ITNBR");

                var vRtn3 = this.gf_SelectSql_sync("ds_temp: SELECT ITTYP FROM ITEMAS WHERE ITNBR = '" + vs_Itnbr + "' ", "SELECT_reffpf_ittyp", "ff_Callback_sync", 0);

                if (vRtn3[1] == '7')//상품 금액계산.
                {
                    nComm_amount = nComm_amount + this.ds_detail.getColumn(i, "ORDER_AMT");
                    nIttypCount = nIttypCount + 1;
                }
            }

            if (nIttypCount > 0 && nComm_amount > nAmount_chul) {
                alert("상품출고 가능 금액 초과입니다. 상품 출고 가능 잔액 = " + nComm_amount_org);
                return -1;
            }
        }
...
this.ff_Depot = function (vEmpId) {
    ...
    //출고 창고 셋팅
    var vSql = "SELECT    A.CVCOD AS CODE, B.CVNAS AS DATA, A.SEQ    "
        + "       FROM     "
        + "         (     "
        + "                 SELECT    A.CVCOD, A.JUMAECHUL	, DECODE(A.CVCOD,'ZA010','1','99') AS SEQ	"
        + "                   FROM    VNDMST_STOCK A     "
        + "                  WHERE    A.DEPTCODE LIKE '" + vDeptcode + "'      "
        + "                 AND   A.SOYOUJA = '1'     "
        + "                 AND   A.JUMAECHUL <> '8'   "
        + "                  UNION    ALL     "
        + "                 SELECT    A.CVCOD, B.JUMAECHUL , DECODE(A.CVCOD,'ZA010','1','99') AS SEQ	   "
        ...
        + "     GROUP BY A.CVCOD, A.JUMAECHUL, B.CVNAS , A.SEQ  "
...
this.ff_Danga = function (vItnbr, vRow, vTuncu, vCalcu) {
    ...
    // 2021.12.20 제품PM팀에서 요청한 내용 추가(IS-III 전용 PKG에서 품목정보에 지정한 품목은 주문 안되게)
    var vRtn = this.gf_SelectSql_sync("ds_temp: SELECT TELNO1 FROM ESTIMATE_HEAD WHERE ESTNO = '" + vEstno + "' ", "SELECT_ESTIMATE_HEAD", "ff_Callback_sync", 0);

    if (!NXCore.isEmpty(vRtn[1]) && vRtn[1] == 'Y') {
        var vRtn2 = this.gf_SelectSql_sync("ds_temp: SELECT UM_GUBUN FROM ITEMAS WHERE ITNBR = '" + vItnbr + "' ", "SELECT_ITEMAS", "ff_Callback_sync", 0);

        if (!NXCore.isEmpty(vRtn2[1]) && vRtn2[1] == 'Y') {
            alert('IS-III 전용 PKG에서는 IS-II 주문 불가!!');
...
//------------------------------------------------------------------
//NI 잔여계약 확인
this.ff_Package_Ni = function (vCvcod, vCon_spc_gbn) {

    var v_est_jan, v_estno;
    var vSql_ni = "SELECT A.ESTNO, A.ESTSTS, "
        + " (SELECT C.AMOUNT - (C.USE_CON_AMOUNT + C.USE_SOBIJA_AMOUNT) + (C.USE_BACK_CON_AMOUNT + C.USE_BACK_SOBIJA_AMOUNT) FROM TABLE ( PKG_SALE_014.PKG_SALE_014_EST('V03N00'||A.CVCOD||'^'||A.ESTNO||'^2^')) C ) AS TOTAL_JAN "
        + " FROM ESTIMATE_HEAD A "
        + " WHERE A.CVCOD = '" + vCvcod + "' AND A.CON_SPC_GBN = '" + vCon_spc_gbn + "' AND A.ESTSTS IN ('0','1') ";

    var vRtn_ni = this.gf_SelectSql_sync("ds_temp:" + vSql_ni, "SELECT_NI_ESTIMATE", "ff_Callback_sync", 3);

    if (vi_ErrorCode < 0) return false;

    if (this.ds_temp.rowcount > 0) {
        for (i = 0; i < this.ds_temp.rowcount; i++) {
            v_estno = this.ds_temp.getColumn(i, 'ESTNO');

            if (this.ds_temp.getColumn(i, 'ESTSTS') == '0')//잔여계약이 접수 상태일 경우 바로 리턴
            {
                return v_estno;
            }
            else //사용중인 잔여계약에 대한 잔액 확인
            {
                v_est_jan = this.ds_temp.getColumn(i, 'TOTAL_JAN');

                var vSql_ni_2 = "SELECT NVL(SUM(A.ORDER_AMT), 0) AS ORDER_AMT"
                    + " FROM SORDER A, IOMATRIX B "
                    + " WHERE A.CVCOD = '" + vCvcod + "' AND A.ESTNO = '" + v_estno + "' AND A.OUT_GU = B.IOGBN AND B.TYPGBN IN ('1', '3')"
                    + " AND B.TRAGBN NOT IN ('8') AND DEPOT_NO NOT IN ('ZA161') AND A.SUJU_STS NOT IN ('4','8')";

                var vRtn_ni_2 = this.gf_SelectSql_sync("ds_temp:" + vSql_ni_2, "SELECT_EST_ORDER", "ff_Callback_sync", 3);


                var vs_acount = v_est_jan - this.ds_temp.getColumn(0, "ORDER_AMT");
                if (vs_acount > 0) //잔액 남아있을경우
                {
                    return v_estno;
                }
                else  //미출고 주문 포함 잔액 모두 소진되었을 경우
                {
                    return false;
                }
            }// end if
...
this.ff_Package_Reset = function (vCvcod, vEstno) {
    ...
    if (vResult[2] == '1') {
        ...
        if (vResult[2] == '2') {
            if (vResult[26] == '0') {
                ...
                //NI 잔여계약 확인
                var vSql_ni = "SELECT A.CON_SPC_GBN FROM ESTIMATE_HEAD A "
                    + " WHERE A.ESTNO = '" + vResult[3] + "' AND A.ESTSTS = '1'";
                var vRtn_ni = this.gf_SelectSql_sync("ds_temp:" + vSql_ni, "SELECT_ESTIMATE_HEAD", "ff_Callback_sync", 0);
                if (vi_ErrorCode < 0) return false;

                if (vRtn_ni[1] != '6') // 잔여 계약 체크 로직에서 소액패키지는 제외한다(2022.04.27)
                {
                    // 20240403 NI 잔여계약 체크 로직 주석처리 - 시작
                    /*
                       var return_ni = this.ff_Package_Ni(vCvcod,vRtn_ni[1]);
        
                       if(return_ni)
                       {
                            alert("NI에 이미 사용중인 계약이 있습니다. 계약번호 : " + return_ni);
                            this.ds_master.setColumn(0, "ESTNO", null);
                            this.ds_package.clearData();
                            this.ds_package.insertRow(0);
                            this.gsi_dataset_zero_set(this.ds_package, 0, "GUBUN^3");
                ...
                vSql = "SELECT TAX_GBN4, CON_SPC_GBN, FO_SUDAT, B.PKGGU, B.PKGNAME, C.PRICE_TYPE, A.CON_NEW_GBN, A.CONDAT, A.ESTSTS , A.PARTIAL_CONDITION"
                ...
                if (vRtn[7] != '1' && vRtn[1] != 'Y' && vResult[26] == 1 && (nChul - nBack >= nSugum + nPrv) &&
                    vRtn[2] != '6' && vRtn[2] != '8' && ivAllow != 'Y' && vRtn[10] != 'Y') {
    ...
    // 20240611 계약 패키지마스타 단가 수정 제한 여부 로직 추가 -- 끝
    var vsql = "SELECT RFNA1 FROM REFFPF WHERE RFCOD = '5U' AND RFGUB = '" + vCon_spc_gbn + "' ";

    var vRtn1 = this.gf_SelectSql_sync("ds_temp:" + vsql, "SELECT_reffpf_5U", "ff_Callback_sync", 0);
    if (vi_ErrorCode < 0) return false;

    this.Div_content.DIV_package.ed_conspcgbn.set_value(this.ds_temp.getColumn(0, "RFNA1"));
    ...
    //////////////////////////////////////////////////////////////////////
    // 20230705 쇼핑몰 패키지 추가
    if (this.ds_package.getColumn(0, "CON_SPC_GBN") == 'B' && ivChk == '1') {

        var vSql, vRtn;

        vSql = "SELECT COUNT(*) AS CNT "
            + "  FROM SUB2_USER_T "
            + " WHERE MAIN_ID = '51' AND SUB1_ID = '40' AND SUB2_ID = '80' AND L_USERID = '" + ivUserid + "' ";
        vRtn = this.gf_SelectSql_sync("ds_temp:" + vSql, "SELECT_SUB2_USER_T", "ff_Callback_sync", 0);

        if (vi_ErrorCode < 0) return false;

        if (this.ds_temp.getColumn(0, "CNT") == '0') {
            this.gf_mdi_btn_disable("save");

            this.Div_content.set_enable(false);
            this.Div_detail.btn_detail_add.set_enable(false);
            this.Div_detail.btn_detail_delete.set_enable(false);
            this.Div_detail.btn_detail_add00.set_enable(false);
            this.Div_detail.btn_detail_delete00.set_enable(false);
        }
...
this.ff_Package_Reset_Inquery = function (vCvcod, vEstno) {
    ...
    if (vResult[2] == '1') {
        ...
        if (vResult[26] == '0') {
            ...
            vSql = "SELECT TAX_GBN4, CON_SPC_GBN, FO_SUDAT, B.PRICE_TYPE, CON_NEW_GBN, CONDAT, ESTSTS, A.PARTIAL_CONDITION FROM ESTIMATE_HEAD A, ESTIMATE_HEAD_RMK B "
            ...
            if (vRtn[5] != '1' && vRtn[1] != 'Y' && vResult[26] == 1 && (nChul - nBack > nSugum + nPrv) &&
                vRtn[2] != '1' && vRtn[2] != '6' && vRtn[2] != '8' && vRtn[8] != 'Y') {
...
this.ff_Danpum_Limit = function (vCvcod) {
    var nLimit = parseFloat(this.gf_Getsyscnfg('S', 18, 1));
    var vs_Order_date = this.ds_master.getColumn(0, "ORDER_DATE");

    ///////////////////////////////////////////////////////////////////
    ///// 거래처별 특별 적용한도가 설정되어 있으면 우선 적용한다. /////    
    ///////////////////////////////////////////////////////////////////
    var vRtn = this.gf_SelectSql_sync("ds_temp: SELECT NVL(SUM(VALIDAMT), 0) FROM VNDDAMBO WHERE CVCOD = '" + vCvcod + "' AND GUBUN = '1' AND ALLOWDT >= '" + vs_Order_date + "' ", "SELECT_VNDDAMBO", "ff_Callback_sync", 0);

    if (vi_ErrorCode < 0) return false;

    var vRtn2 = this.gf_SelectSql_sync("ds_temp: SELECT NVL(SUM(ETCAMT), 0) FROM VNDDAMBO WHERE CVCOD = '" + vCvcod + "' AND GUBUN = '2'", "SELECT_VNDDAMBO", "ff_Callback_sync", 0);
    ...
    var vRtn = this.gf_SelectSql_sync("ds_temp: SELECT NVL(SUM (A.IOAMT * B.CALVALUE), 0) AS AMT "
        ...
        + "   AND B.TYPGBN IN ('1', '3') AND B.TRAGBN NOT IN ('8') AND DEPOT_NO NOT IN ('ZA161') AND A.IO_DATE >= '20220401'", "SELECT_IMHIST_SAL", "ff_Callback_sync", 0);
    if (vi_ErrorCode < 0) return false;
    ...
    var vRtn = this.gf_SelectSql_sync("ds_temp: SELECT NVL(SUM(A.ORDER_AMT), 0) AS ORDER_AMT "
        ...
        + "   AND B.TRAGBN NOT IN ('8') AND DEPOT_NO NOT IN ('ZA161') AND A.SUJU_STS NOT IN ('4','8') AND A.ORDER_DATE >= '20220401'", "SELECT_SORDER", "ff_Callback_sync", 0);
    if (vi_ErrorCode < 0) return false;
    var nAmt2 = parseFloat(vRtn[1]);

    var vRtn = this.gf_SelectSql_sync("ds_temp: SELECT NVL(SUM(A.IPGUM_AMT), 0) AS IPGUM_AMT "
        + "  FROM SUGUM A "
        + " WHERE A.CVCOD = '" + vCvcod + "' AND A.ESTNO IS NULL AND A.IPGUM_DATE >= '20220401'", "SELECT_SORDER", "ff_Callback_sync", 0);

    var nAmt3 = parseFloat(vRtn[1]);

    var nOrder_amt = this.gd_detail.getSummValue(8);

    if ((nAmt1 + nAmt2) - nAmt3 - nAmt4 > nLimit) {
        ...
        return (nAmt1 + nAmt2) - nAmt3 - nAmt4;
}


this.ff_Danpum_Limit2 = function (vCvcod) {
    var nLimit = parseFloat(this.gf_Getsyscnfg('S', 18, 1));
    var vs_Order_date = this.ds_master.getColumn(0, "ORDER_DATE");
    ...
    if ((nAmt1 + nAmt2 + nOrder_amt) - nAmt3 - nAmt4 > nLimit) {
        alert(" 현재 단품출고 금액이 허용 한도를 초과합니다.");
            ...
            if (vRtn[1] == 'Y') {
                ...
                if (vRtn[3] == 'Y') {
                    if (this.gf_today() > vRtn[4] || this.ds_master.getColumn(0, "ORDER_DATE") > vRtn[4]) {
                        alert(" 해당 계약은 한시적 출고 허용일자가 경과하여 주문 불가합니다1.");
...
this.ff_AfterPopup = function (strId, obj) {
    ...
    switch (strId) {
        ...
        case "estimate_popup":
            ...
            if (va_Data != false) {
                for (var i = 0; i < va_Data.length; i++) {
                    var vCon_spc_gbn = va_Data[i][25];
                    this.Div_content.DIV_package.ed_conspcgbn.set_value(va_Data[i][26]);
                ...
                ////단품 출고제한 로직 (패키지 잔액 100만원 이상 단품 출고 금지.)
                var vSql3 = "SELECT A.ESTNO, A.USE_SUGUM_AMOUNT - ((A.USE_PRICE_AMOUNT + A.USE_CON_AMOUNT) - (A.USE_BACK_PRICE_AMOUNT + A.USE_BACK_CON_AMOUNT)) "
                    + "	- ( SELECT NVL(SUM(ORDER_AMT),0) FROM SORDER WHERE ESTNO = A.ESTNO AND SUJU_STS <> '8') AS JAN_AMT "
                    + " FROM ESTIMATE_HEAD A WHERE A.CVCOD = '" + vs_cvcod + "' AND A.CON_SPC_GBN IN ('1','11','9') AND A.ESTSTS = '1' ";
                var vRtn3 = this.gf_SelectSql_sync("ds_temp:" + vSql3, "SELECT_PACKAGE_JAN", "ff_Callback_sync", 0);

                for (var i = 0; i < this.ds_temp.rowcount; i++) {
                    if (this.ds_temp.getColumn(i, 'JAN_AMT') >= 1000000) {
                        alert("패키지 계약중 잔액이 100만원 이상인 계약이 있어 단품 출고 불가합니다! 계약번호 = " + this.ds_temp.getColumn(i, 'ESTNO'));
                        this.btn_cancel_onclick();
                        return;
                    }

                }//end for 단품출고 제한 로직 end
            ...
            for (var i = 0; i < va_Data.length; i++) {
                ...
                if (NXCore.isEmpty(this.ds_master.getColumn(0, 'ESTNO')) && this.ds_master.getColumn(0, 'SAREA') != '01' && vs_deptcode != 'P125000')//단품 출고제한 로직 (단품 주문시 상품, KIT 제품은 주문 불가)
                {
                    ...
                    if (vs_Ittyp == '7') {
                        ...
                        v_danga = this.ff_Danga_re(va_Data[i][0], vn_Row, 'KRW', 'Auto');
            ...
            for (var i = 0; i < va_Data.length; i++) {
                ...
                if (NXCore.isEmpty(this.ds_master.getColumn(0, 'ESTNO')) && this.ds_master.getColumn(0, 'SAREA') != '01' && vs_deptcode != 'P125000')//단품 출고제한 로직 (단품 주문시 상품, KIT 제품은 주문 불가)
                {
                    ...
                    if (vRtn[5] == '7') {
                        ...
                        v_danga = this.ff_Danga_re(va_Data[i][2], vRow, 'KRW', 'Auto');
                ...
                var vSql = "SELECT	COUNT(*) AS CNT FROM REFFPF WHERE RFCOD = '2L' AND RFGUB <> '00' AND RFNA1 = '" + va_Data[i][2] + "' ";
                var vRtn = this.gf_SelectSql_sync("ds_temp:" + vSql, "SELECT_reffpf_2L", "ff_Callback_sync", 0);
                if (vi_ErrorCode < 0) return false;
...
/// 기준단가 return 
///
this.ff_Danga_re = function (vItnbr, vRow, vTuncu, vCalcu) {
    // vCalcu : 강제로 소비자가 계산
